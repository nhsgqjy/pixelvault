...
function Root(){const[auth,setAuth]=useState<boolean|null>(null);useEffect(()=>{api.raw('/auth/me').then(response=>setAuth(response.ok)).catch(()=>setAuth(false))},[]);if(auth===null)return <main className="login-page"><Sparkles/></main>;return auth?<VaultPage onLogout={()=>setAuth(false)}/>:<LoginPage onLogin={()=>setAuth(true)}/>}
