...
export function MobileNavigation({view, onSwitch, onLogout}: Props) {
  const mobile = nav.filter(([key]) => ['photos', 'timeline', 'favorites', 'albums', 'insights'].includes(key));
  return <nav className="mobile-nav" aria-label="Primary navigation">{mobile.map(([key, Icon, label]) =>
