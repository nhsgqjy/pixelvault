...
export function SharePage({token}: {token: string}) {
  ...
  useEffect(() => {
    api.raw(`/share/${token}`).then(response => response.ok ? response.json() : Promise.reject())
      .then(setPhoto).catch(() => setPhoto(null));
  }, [token]);
  return <main className="share-page"><div className="brand dark"><Sparkles/>PixelVault</div>{photo ? <>
    <p>SHARED MEMORY</p><h1>{photo.name}</h1><img src={api.url(`/share/${token}/content`)}/>
...
export function AlbumSharePage({token}: {token: string}) {
  ...
  useEffect(() => {
    api.raw(`/share/albums/${token}`).then(response => response.ok ? response.json() : Promise.reject())
  ...
  return <main className="shared-album-page"><header><div className="brand dark"><Sparkles/>PixelVault</div>{album && <small>{album.views} visits · expires {new Date(album.expires_at).toLocaleDateString()}</small>}</header>{album ? <>
    ...
    <section className="shared-album-grid">{album.photos.map(photo => <button key={photo.id} onClick={() => setSelected(photo)}><img src={api.url(`/share/albums/${token}/photos/${photo.id}/thumbnail`)} alt={photo.caption || photo.name}/><span>{photo.caption || photo.name}</span></button>)}</section>
    {selected && <div className="shared-lightbox" onClick={() => setSelected(null)}><button aria-label="Close">×</button><img onClick={event => event.stopPropagation()} src={api.url(`/share/albums/${token}/photos/${selected.id}/content`)} alt={selected.caption || selected.name}/></div>}
