...
export function VaultPage({onLogout}:{onLogout:()=>void}){
 ...
 const{performAction,action,batch,batchCaptureDate,createAlbum,createNewAlbum,renameCurrent,editAlbumDescription,setAlbumCover,removeFromCurrentAlbum,retryProcessing,deleteCurrent,exportSelection,exportAlbum,shareAlbum,revokeAlbumShare}=useVaultActions({checked,setChecked,activeAlbum,setActiveAlbum,setConfirming,setNotice,setPhotos,load,loadAlbums});
 ...
 return <div className="app"><Sidebar view={view} onSwitch={switchView} onLogout={lockVault}/><main>{confirming&&<DeleteDialog intent={confirming} onCancel={()=>setConfirming(null)} onConfirm={async()=>{const intent=confirming;setConfirming(null);const succeeded=await performAction(intent.id,intent.kind);if(!succeeded)return;if(intent.kind==='trash'&&activeAlbum)setActiveAlbum(current=>current?{...current,photo_count:Math.max(0,current.photo_count-1)}:current);await loadAlbums()}}/>}
