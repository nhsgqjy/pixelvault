...
export function VaultPage({onLogout}:{onLogout:()=>void}){
 ...
 {view==='photos'&&<section className={`drop ${drag?'active':''}`} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);accept(e.dataTransfer.files)}}><CloudUpload/><b>Drop photos anywhere to upload</b><span>3 concurrent chunks · retry-safe · resumable</span></section>}<UploadQueue progress={progress} uploadState={uploadState} speeds={speeds} onToggle={toggleUpload}/>
